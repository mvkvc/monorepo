...
const byesig = (function () {
  ...
  const RE_SIG_BLOCK = "^ *sig do$.*?^ *ends*$";
  ...
  function decorationRenderOption() {
    ...
    if (vscode.workspace.getConfiguration("byesig").get("showIcon")) {
      decoration["gutterIconPath"] = path.join(
        __dirname,
        "..",
        "misc",
        "icon.png"
  ...
  function hideSig() {
    ...
    byesigDecorationType[tile_key] =
      vscode.window.createTextEditorDecorationType(decorationRenderOption());
  ...
  async function foldSig(force = false) {
    ...
    setTimeout(async () => {
      ...
      let folded_selections = getMatchPositions(
        new RegExp(RE_SIG_BLOCK, "gsm"),
        editor
      ).map((range) => {
  ...
  function activate(context) {
    context.subscriptions.push(
      vscode.commands.registerCommand("byesig.hideSig", onCommandHideAndFoldSig)
    );
    context.subscriptions.push(
      vscode.commands.registerCommand(
        "byesig.showSig",
        onCommandShowAndUnfoldSig
      )
    );
